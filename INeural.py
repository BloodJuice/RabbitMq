...
#Принимает строку текста для перевода, опционально: (строку ключа языка, с которого осуществляется перевод, скажем "en", если не передавать, то определит язык источника автоматически. Если он будет совпадать с языком назвачения - вернёт строку буз перевода) и (строку ключа языка, на который осуществляется перевод, скажем "en", если не указывать, по умолчанию переводит на английский).
# Возвращает кортеж из строки ключа языка источника (по нему можно определить, был ли произведён перевод, или он равен ключу языка назвачения,
# и с какого языка перевод осуществлялся, если язык источника не был указан) и строки переведённого текста
...
#Принимает первое и второе изображения для совмещения, описание первого и второго изображения для совмещения и словарь параметров.
